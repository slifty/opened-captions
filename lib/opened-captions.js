...
module.exports = (function() {
  ...
  OpenedCaptions.prototype.processContent = function(content, stream) {
    ...
    // If the content has whitespace, send the appropriate word message
    if(/\s/.test(content)) {

      // Extract the most recent word from the buffer
      var buffer = stream.getBuffer();
      var splitBuffer = buffer.split(/\s+/);
      var word = splitBuffer.pop();

      // If the word exists, send it
      if(word) {
        var wordMessage = new Message('word', word, stream);
        this.sendMessage(wordMessage);
      }
    }
  }
